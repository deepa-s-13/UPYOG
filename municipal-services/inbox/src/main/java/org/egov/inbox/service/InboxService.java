...
//        Integer nearingSlaProcessCount = workflowService.getNearingSlaProcessCount(criteria.getTenantId(), requestInfo, processCriteria);
		...
		if (!(processCriteria.getModuleName().equals(PTR) || processCriteria.getModuleName().equals(PT)
				|| processCriteria.getModuleName().equals(ASSET) || processCriteria.getModuleName().equals(EWASTE))) {
		...
		if (!CollectionUtils.isEmpty(moduleSearchCriteria)) {
			...
			if (!ObjectUtils.isEmpty(processCriteria.getModuleName())
					&& processCriteria.getModuleName().equals(EWASTE)) {
			...
			*/
			...
			if (!ObjectUtils.isEmpty(processCriteria.getModuleName())
				...
				try {
					responseNode = new ObjectMapper().convertValue(
							elasticSearchRepository.elasticSearchApplications(criteria, (List<String>) null),
							JsonNode.class);
...
//                    }
					...
					if (!isNull(output) && output.isArray()) {
						for (JsonNode objectnode : output) {
							...
							Long applicationServiceSla = getApplicationServiceSla(businessServiceSlaMap,
									data.get("Data"));
					...
					throw new CustomException("ELASTICSEARCH_ERROR",
							"client error while searching ES : \" + e.getMessage()");
				}
			}
